...
import net.watchpeople.domain.box.repository.BoxShareRequestRepository;
...
public class SharedBoxRequestService {
    ...
    private final BoxShareRequestRepository boxShareRequestRepository;

    // 공유 박스 신청 (PENDING)
    @Transactional
    public void requestSharedBox(Member sender, Member receiver) {
        boxShareRequestRepository.save(BoxShareRequest.builder()
                .sender(sender)
                .receiver(receiver)
    ...
    public List<BoxShareRequestResponse> getSharedBoxRequests(Member member) {
        return boxShareRequestRepository.findByReceiverAndStatus(member, RequestStatus.PENDING).stream()
