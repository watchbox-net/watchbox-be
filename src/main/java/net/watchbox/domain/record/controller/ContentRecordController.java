...
@Tag(name = "ContentRecord", description = "컨텐츠에 대한 시청 상태, 좋아요 API")
public class ContentRecordController {
    ...
    @GetMapping("/status")
    public ResponseEntity<ApiResponse<ContentRecordPageResponse>> getWatchStatusList(
...
//                watchRecordFacade.getWatchStatusList(member, mediaType, watchStatus, page, size))
    ...
    @Operation(summary = "좋아요 표시된 시청 기록 리스트 조회",
            description = "좋아요는 시청 상태와 달리 PERSON도 포함하여 조회 <br>"+
                    "좋아요만 포함, 싫어요는 미포함")
    @GetMapping("/likes")
    public ResponseEntity<ApiResponse<ContentRecordPageResponse>> getContentLikeList(
            @AuthenticationPrincipal Member member
//            @RequestParam(value = "mediaType", required = false) String mediaType,
//            @RequestParam(value = "page", defaultValue = "0") int page,
//            @RequestParam(value = "size", defaultValue = "10") int size
