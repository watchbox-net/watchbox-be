...
public class BoxMemberService {
    ...
    public void validateSharedBoxContentAdder(Box box, Member member) {
        // 해당 BoxMember 인지
    ...
    public void validateSharedBoxContentRemover(Box box, Member member, SharedBoxContent sharedBoxContent) {
        // BoxContent 추가한 회원인지
    ...
    public void validateSharedBoxOwner(Box box, Member member) {
        // 해당 BoxMember 인지
        BoxMember boxMember = boxMemberRepository.findByBoxAndMember(box, member)
                .orElseThrow(() -> new CustomException(ErrorCode.NOT_BOX_MEMBER, member.getMemberId(), "Member"));
        // Role이 OWNER인지
